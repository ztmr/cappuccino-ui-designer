...
- (CPPredicate)parse
{
    ...
    {
        CPLog(@"Unable to parse predicate '" + [self string] + "' with " + error);
    }
    finally
    {
        if (![self isAtEnd])
        {
            ...
            CPLog(@"Format string contains extra characters: '" + [pstr substringToIndex:loc] + "**" + [pstr substringFromIndex:loc] + "**'");
        }
    }
