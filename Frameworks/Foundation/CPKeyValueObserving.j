...
- (id)initWithTarget:(id)aTarget
{
    if (self = [super init])
    {
        ...
        self._nativeClass        = [aTarget class];
...
- (void)_replaceClass
{
    ...
    if ([self._targetObject isKindOfClass:[CPDictionary class]])
...
- (void)_replaceModifiersForKey:(CPString)aKey
{
    ...
    if ([self._replacedKeys containsObject:aKey] || ![self._nativeClass automaticallyNotifiesObserversForKey:aKey])
        return;

    [self._replacedKeys addObject:aKey];
    ...
    var affectingKeys = [[self._nativeClass keyPathsForValuesAffectingValueForKey:aKey] allObjects],
    ...
    while (affectingKeysCount--)
    {
        ...
        if (affectingKey.indexOf(@".") !== -1)
            [self._targetObject addObserver:self forKeyPath:affectingKey options:CPKeyValueObservingOptionPrior | kvoNewAndOld  context:nil];
        else
...
- (void)_addObserver:(id)anObserver forKeyPath:(CPString)aPath options:(unsigned)options context:(id)aContext
{
    ...
    if (aPath.indexOf('.') !== CPNotFound && aPath.charAt(0) !== '@')
        forwarder = [[_CPKVOForwardingObserver alloc] initWithKeyPath:aPath object:self._targetObject observer:anObserver options:options context:aContext];
    else
    ...
    {
        var newValue = [self._targetObject valueForKeyPath:aPath];

        if (newValue === nil || newValue === undefined)
            newValue = [CPNull null];

        var changes = @{ CPKeyValueChangeNewKey: newValue };
        [anObserver observeValueForKeyPath:aPath ofObject:self._targetObject change:changes context:aContext];
    }
...
- (void)_removeObserver:(id)anObserver forKeyPath:(CPString)aPath
{
    ...
    {
        self._targetObject.isa = self._nativeClass; //restore the original class
...
- (void)_sendNotificationsForKey:(CPString)aKey changeOptions:(CPDictionary)changeOptions isBefore:(BOOL)isBefore
{
    ...
    if (isBefore)
    {
        if (changes)
        {
            ...
            if (!level)
                [CPException raise:CPInternalInconsistencyException reason:@"self._changesForKey without self._nestingForKey"];
        ...
        if (setMutationKind)
        {
            var setMutationObjects = [changes[_CPKeyValueChangeSetMutationObjectsKey] copy],
                setExistingObjects = [[self._targetObject valueForKey: aKey] copy];
        ...
        else if (indexes)
        {
            ...
            if (type === CPKeyValueChangeReplacement || type === CPKeyValueChangeRemoval)
            {
                ...
                var newValues = [[self._targetObject mutableArrayValueForKeyPath:aKey] objectsAtIndexes:indexes];
                [changes setValue:newValues forKey:CPKeyValueChangeOldKey];
            }
        }
        else
        {
            var oldValue = [self._targetObject valueForKey:aKey];
    ...
    else
    {
        ...
        {
            var newValue = [self._targetObject valueForKey:aKey];
    ...
    var observers = [self._observersForKey[aKey] allValues],
    ...
    while (count--)
    {
        ...
        if (!isBefore || (observerInfo.options & CPKeyValueObservingOptionPrior))
            [observerInfo.observer observeValueForKeyPath:aKey ofObject:self._targetObject change:changes context:observerInfo.context];
    }
    ...
    for (; index < count; ++index)
    {
        ...
        [self _sendNotificationsForKey:keyPath
                         changeOptions:isBefore ? [changeOptions copy] : self._changesForKey[keyPath]
                              isBefore:isBefore];
    }
}
...
- (id)initWithKeyPath:(CPString)aKeyPath object:(id)anObject observer:(id)anObserver options:(unsigned)options context:(id)aContext
{
    ...
    [self._object addObserver:self forKeyPath:self._firstPart options:self._options context:nil];

    //the current value of a (not the value of a.b)
    self._value = [self._object valueForKey:self._firstPart];

    if (self._value)
        [self._value addObserver:self forKeyPath:self._secondPart options:self._options context:nil]; //we're observing b on current a
...
- (void)observeValueForKeyPath:(CPString)aKeyPath ofObject:(id)anObject change:(CPDictionary)changes context:(id)aContext
{
    ...
    {
        var oldValue = [self._value valueForKeyPath:self._secondPart],
            newValue = [self._object valueForKeyPath:self._firstPart + "." + self._secondPart],
            pathChanges = @{
        ...
        [self._observer observeValueForKeyPath:self._firstPart + "." + self._secondPart ofObject:self._object change:pathChanges context:self._context];

        //since a has changed, we should remove ourselves as an observer of the old a, and observe the new one
        if (self._value)
            [self._value removeObserver:self forKeyPath:self._secondPart];

        self._value = [self._object valueForKey:self._firstPart];

        if (self._value)
            [self._value addObserver:self forKeyPath:self._secondPart options:self._options context:nil];
    }
    else
    {
        //a is the same, but a.b has changed -- nothing to do but forward this message along
        [self._observer observeValueForKeyPath:self._firstPart + "." + aKeyPath ofObject:self._object change:changes context:self._context];
    }
}
...
- (void)finalize
{
    ...
    [self._object removeObserver:self forKeyPath:self._firstPart];
