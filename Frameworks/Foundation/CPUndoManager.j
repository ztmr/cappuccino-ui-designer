...
+ (id)undoGroupingWithParent:(_CPUndoGrouping)anUndoGrouping
{
    if (_CPUndoGroupingPool.length)
    {
        ...
        if (grouping._invocationss.length)
            grouping._invocationss = [];
...
{
    _invocationss.push(anInvocation);
}

- (void)addInvocationsFromArray:(CPArray)invocations
{
    [_invocationss addObjectsFromArray:invocations];
}

- (BOOL)removeInvocationsWithTarget:(id)aTarget
{
    var index = _invocationss.length;

    while (index--)
        if ([_invocationss[index] target] == aTarget)
            _invocationss.splice(index, 1);
}

- (CPArray)invocations
{
    return _invocationss;
}
...
- (id)initWithCoder:(CPCoder)aCoder
{
    ...
    if (self)
    {
        ...
        _invocationss = [aCoder decodeObjectForKey:_CPUndoGroupingInvocationsKey];
...
{   
    ...
    [aCoder encodeObject:_invocationss forKey:_CPUndoGroupingInvocationsKey];
